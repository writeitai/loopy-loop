...
| `recovery_policy` | string | `drain` | What crash recovery does with agent processes a dead worker left running: `drain` lets them finish (bounded by `recovery_drain_timeout_s`) before the iteration is re-run; `reap` kills them immediately. |
| `recovery_drain_timeout_s` | number | `600` | Shared deadline in seconds for draining orphaned agents during crash recovery. Must be `0` or greater. |
